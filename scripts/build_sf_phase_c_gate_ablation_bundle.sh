...
archive="${1:-${workspace}/artifacts/sf_phase_c_gate_ablation_20260818.tar.gz}"
stage="$(mktemp -d)"
...
declare -a files=(
  ...
  "scripts/install_sf_phase_c_gate_ablation_on_server.sh"
)
...
sha256sum "${archive}"
ls -lh "${archive}"
echo "SF_PHASE_C_GATE_ABLATION_BUNDLE=PASS"
