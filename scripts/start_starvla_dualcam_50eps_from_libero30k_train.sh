...
# Scheme A: initialize a fresh optimizer from the original dual-camera LIBERO
# 30k model, then adapt directly to the merged 50-episode real Franka dataset.
# This deliberately does not load any previous single-camera Franka checkpoint.
...
for required in \
  ...
  "${dataset_path}/meta/modality.json"; do
  if [[ ! -f "${required}" ]]; then
...
if [[ -e "${output_dir}" ]]; then
  echo "Refusing to overwrite existing Scheme A output: ${output_dir}" >&2
  exit 1
fi
mkdir -p "${run_root}"
...
echo "SCHEME=A_LIBERO30K_TO_REAL_DUALCAM50"
echo "Dataset: ${dataset_path}"
echo "Pretrained checkpoint: ${checkpoint}"
echo "Previous single-camera Franka checkpoint: NOT USED"
