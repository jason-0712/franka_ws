...
archive="${1:-/tmp/spatial_forcing_phase8_relational_20260804.tar.gz}"
stage_root="$(mktemp -d)"
...
for script_name in \
  install_spatial_forcing_phase8_relational.sh \
  run_spatial_forcing_phase8_tests_on_server.sh; do
  install -D -m 0755 \
...
sha256sum "${archive}"
ls -lh "${archive}"
echo "SPATIAL_FORCING_PHASE8_BUNDLE=${archive}"
