...
declare -a mappings=(
  ...
  "scripts/finalize_phase_d081_sam2_review.py:finalize_phase_d081_sam2_review.py:0755"
  "scripts/test_audit_phase_d081_sam2_bidirectional.py:tests/test_audit_phase_d081_sam2_bidirectional.py:0755"
  "scripts/test_finalize_phase_d081_sam2_review.py:tests/test_finalize_phase_d081_sam2_review.py:0755"
...
for mapping in "${mappings[@]}"; do
  ...
  if [[ -e "${target}" ]]; then
    cp -a "${target}" "${target}.before_phase_d081_${timestamp}"
  fi
...
"${python_bin}" -m py_compile \
  ...
  finalize_phase_d081_sam2_review.py \
  tests/test_audit_phase_d081_sam2_bidirectional.py \
  tests/test_finalize_phase_d081_sam2_review.py
PYTHONPATH="${repo}:${PYTHONPATH:-}" \
  "${python_bin}" tests/test_audit_phase_d081_sam2_bidirectional.py
PYTHONPATH="${repo}:${PYTHONPATH:-}" \
  "${python_bin}" tests/test_finalize_phase_d081_sam2_review.py

echo "PHASE_D081_SAM2_BIDIRECTIONAL_INSTALL=PASS"
echo "ROBOT_COMMANDS_SENT=0"
echo "RUNNER=${repo}/run_phase_d081_sam2_bidirectional_on_server.sh"
