...
class PolicyNormProcessor:
    ...
    def apply_state(self, raw_state: np.ndarray) -> np.ndarray:
        """Normalize raw state using the exact training-time transform.

        ``raw_state`` may be shaped ``(D,)`` or ``(T, D)``. The returned
        array is always ``(T, D)`` and follows ``state_keys`` order.
        """
        raw_state = np.asarray(raw_state, dtype=np.float32)
        if raw_state.ndim == 1:
            raw_state = raw_state[None, :]
        if raw_state.ndim != 2:
            raise ValueError(f"Expected raw state shape (D,) or (T, D); got {raw_state.shape}")

        expected_dim = sum(self._state_key_dims.get(key, 1) for key in self._state_keys)
        if raw_state.shape[-1] != expected_dim:
            raise ValueError(
                f"Raw state dim {raw_state.shape[-1]} != expected {expected_dim}; "
