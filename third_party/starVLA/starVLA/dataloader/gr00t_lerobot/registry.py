...
def _find_registry_dirs() -> list[Path]:
    ...
    return sorted(
        path
        for path in examples_dir.glob(f"**/train_files/{_REGISTRY_DIR_NAME}")
        if path.is_dir()
    )
...
def discover_and_merge() -> None:
    ...
    for registry_dir in _find_registry_dirs():
        # examples/<BenchName>/train_files/data_registry or
        # examples/<Group>/<BenchName>/train_files/data_registry
        bench_path = registry_dir.relative_to(examples_dir).parents[1]
        bench_name = "_".join(bench_path.parts)
