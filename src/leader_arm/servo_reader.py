...
class ServoReaderNode(Node):
    def __init__(self):
        ...
        self.zero_angles = [0.0] * 8

        # State for interpolation
        self.angle_offset = [0.0] * 8          # currently published angles
        self.target_angle_offset = [0.0] * 8   # target angles
    ...
    def timer_callback(self):
        ...
        for i in range(8):
            ...
            if angle is not None:
                ...
                if i == 7:  # Gripper
                    new_gripper_angle = angle - self.zero_angles[-1]
                    if abs(new_gripper_angle) > 30:
                        print(new_gripper_angle)
                        self.target_angle_offset[i] = 361.0  # close
                    else:
                        self.target_angle_offset[i] = 0.0    # open
                else:
        ...
        for i in range(8):
            if i == 7:
                self.angle_offset[i] =  self.target_angle_offset[i]
            else:
